...
function check_level($btnAction, $isPath = false)
{
    ...
    if ($isPath) {
        ...
        if (in_array('/' . M . '/' . C . '/' . $btnAction, $user_level) || session('id') == 1 || session('id') ==2) {
            return true;
        }
    }
}
...
{
    $user_level = session('levels');
    if (! in_array('/' . M . '/' . C . '/add', $user_level) && session('id') != 1 && session('id') !=2)
        return;
...
function get_btn_del($idValue,$id = 'id', $btnName = '删除')
{
    ...
    if (! in_array('/' . M . '/' . C . '/del', $user_level) && session('id') != 1  && session('id') !=2)
        return;
...
// 获取删除按钮
function get_btn_del2($idValue, $mcode,$id = 'id', $btnName = '删除')
{
    $user_level = session('levels');

    if (! in_array('/' . M . '/' . C . '/del/mcode/'.$mcode, $user_level) && session('id') != 1 && session('id') !=2)
        return;
    $btn_html = "<a href='" . url('/' . M . '/' . C . "/del/$id/$idValue") . "' onclick='return confirm(\"您确定要删除么？\")' class='layui-btn layui-btn-xs layui-btn-danger' title='$btnName'>$btnName</a>";
    return $btn_html;
}
...
// 获取删除按钮
function get_btn_del3($idValue,$id = 'id', $btnName = '删除')
{
    $user_level = session('levels');
...
// 获取撤回按钮
function get_btn_recycle($idValue,$id = 'id', $btnName = '撤回')
{
    $user_level = session('levels');

    if (! in_array('/' . M . '/' . C . '/del', $user_level) && session('id') != 1  && session('id') !=2)
        return;
    $btn_html = "<a href='" . url('/' . M . '/' . C . "/recycle/$id/$idValue") . "' onclick='return confirm(\"您确定要恢复么？\")' class='layui-btn layui-btn-xs layui-btn-normal' title='$btnName'>$btnName</a>";
    return $btn_html;
}
...
{
    $user_level = session('levels');
    if (! in_array('/' . M . '/' . C . '/mod', $user_level) && session('id') != 1 && session('id') !=2)
        return;
...
// 获取修改按钮
function get_btn_mod2($idValue, $scodeValue,$id = 'id', $btnName = '修改')
{
    $user_level = session('levels');
    if (! in_array('/' . M . '/' . C . '/mod', $user_level) && session('id') != 1 && session('id') !=2)
        return;
    $btn_html = "<a href='" . url("/" . M . '/' . C . "/mod/$id/$idValue/scode/$scodeValue") . get_btn_qs() . "'  class='layui-btn layui-btn-xs'>$btnName</a>";
    return $btn_html;
}
