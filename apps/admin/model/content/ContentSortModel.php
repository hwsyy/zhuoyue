...
class ContentSortModel extends Model
{
    ...
    //获取类别所有路径
    public function get_scode_link($pcode,$scode_link=''){
        
       
        if($pcode){
             $scode_link.=$pcode.',';
           // var_dump($pcode);
            $pcode=parent::table('ay_content_sort')->where("scode=$pcode")->value('pcode');
          //  var_dump($scode_link);
          //  var_dump($pcode);
           // exit;
           // $scode_link.=$pcode.',';
           // $this->get_scode_link($pcode,$scode_link);
        }else{
           // var_dump('aaa');
            $scode_link=rtrim($scode_link,',');
      //  var_dump($scode_link);
            $scode_link_arr=explode(',',$scode_link);
            $scode_link_arr_rev=array_reverse($scode_link_arr);
            $scode_link_new=implode(',',$scode_link_arr_rev);

            return $scode_link_new;
        }
        return $this->get_scode_link($pcode,$scode_link);
     //   var_dump($scode_link);
