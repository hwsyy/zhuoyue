...
class ContentSortController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modelExtfield = new ExtFieldModel();
    }
    ...
    private function makeSortList($tree)
    {
        ...
        foreach ($tree as $value) {
            ...
            $this->outData[$this->count]->status_foot = $value->status_foot;
            $this->outData[$this->count]->status_left = $value->status_left;
           
            $this->outData[$this->count]->isrecommend= $value->isrecommend;
        ...
        if ($_POST) {
            if (! ! $multiplename = post('multiplename')) {
                ...
                if(!$pcode){
                    ...
                    $scode_link=$this->model->get_scode_link($pcode);
                    $scode_link='0,'.$scode_link.','.$scode;
                }
                $level=count(explode(',',$scode_link))-1;

                foreach ($names as $key => $value) {
                    $data[] = array(
                        'acode' => session('acode'),
                        'pcode' => $pcode,
                        'scode_link'=>$scode_link,
                        'level'=>$level,
                        'scode' => $scode,
                        'name' => $value,
                        'mcode' => $mcode,
              ...
              //  var_dump($pcode);
                ...
                $status_left=post('status_left');
                $isrecommend=post('isrecommend');
                $subname = post('subname');
                $filename = post('filename');
                $outlink = post('outlink');
                $ico = post('ico');
                $alt_ico=post('ico_alt');
                $pic = post('pic');
                $alt_pic=post('pic_alt');
                $icon=post('icon');
                $icon_white=post('icon_white');
                $pics = post('pics');
    ...
    public function mod()
    {
        ...
        if (($field = get('field', 'var')) && ! is_null($value = get('value', 'var'))) {
           // var_dump($field);var_dump($value);exit;
            if ($this->model->modSort($scode, "$field='$value',update_user='" . session('username') . "'")) {
              //  var_dump($value);var_dump($field);exit;
         ...
         //   var_dump($scode_link);
          ...
          //  exit;
            ...
            if (! $type) {
             ...
             $canshu_array=explode('|',$canshu);
             $sqlite = 'TEXT(1000)';
            foreach($canshu_array as $v){
                $extname='ext_'.$v;
                
                if (! $this->modelExtfield->isExistField($extname)) {

                    $result = $this->modelExtfield->amd("ALTER TABLE ay_content_ext ADD COLUMN $extname $sqlite NULL");
                }
            }
