...
class ContentController extends Controller
{
    ...
    public function index()
    {
        if ((! ! $id = get('id', 'int')) && $result = $this->model->getContent($id)) {
          ...
          //  var_dump($list_select);
            ...
            $this->assign('addscode',session('addscode'));
            // 模型名称
            $this->assign('model_name', model('admin.content.Model')->getName($mcode));

            //当前模型内容
          
            $modelCurrent=model('admin.content.Model')->getCurrent($mcode);
            $this->assign('modelCurrent',$modelCurrent);
            $columnnamearr=explode('，',$modelCurrent->columnname);

            $this->assign('scodename',$columnnamearr[0]);
            $this->assign('titlename',$columnnamearr[1]);
            $this->assign('title2name',$columnnamearr[2]);
            $this->assign('title3name',$columnnamearr[3]);
            $this->assign('title4name',$columnnamearr[4]);
            $this->assign('title5name',$columnnamearr[5]);
            $this->assign('descname',$columnnamearr[6]);
            $this->assign('enclosurename',$columnnamearr[7]);
            $this->assign('contentname',$columnnamearr[8]);
    ...
    public function add()
    {
        ...
        //    $down = post('down');
            $explain=empty($explain)?'':json_encode($explain,JSON_UNESCAPED_UNICODE);
        //    $down=empty($down)?'':json_encode($down,JSON_UNESCAPED_UNICODE);
        //    var_dump($explain);exit;
            // 构建数据
            $data = array(
                'acode' => session('acode'),
                'scode' => $scode,
    ...
    public function mod()
    {
        ...
        if ($_POST) {
           ...
           //     $down=empty($down)?'':json_encode($down,JSON_UNESCAPED_UNICODE);
            ...
            //高级内容
            $explain = json_decode($result->explain);
            //var_dump($explain); 
          //  $down = json_decode($result->down);
            $cont = 0;
         //   $cont2 = 0;
            foreach($explain as $v=>$k){$cont = $v;}
         //   foreach($down as $v=>$k){$cont2 = $v;}
            
            $this->assign('lenexplain', $cont);
            $this->assign('explain', $explain);
         //   $this->assign('lendown', $cont2);
