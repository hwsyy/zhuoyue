...
$(`#btn`).click(function(){
	  ...
	  <div class="layui-inline">
		...
		<div class="layui-input-inline">
		  <input type="tel" name="explain[${num}][title]" autocomplete="off" class="layui-input">
		</div>
	  </div>

	  <div class="layui-inline">
		  <label class="layui-form-label">视频背景图</label>
		   <div class="layui-input-inline">
		   <input type="text" name="explain[${num}][img]" id="img${num}" placeholder="请上传图片"  class="layui-input">
		   </div>
	   <button type="button" class="layui-btn upload${num}" data-des="img${num}">
		<i class="layui-icon">&#xe67c;</i>上传图片
	   </button>
	   <div id="img${num}_box" class="pic addedit" id="del"></div>
	   </div>
	   ...
	   <div class="layui-inline">
		  <label class="layui-form-label">附件</label>
		   <div class="layui-input-inline">
		   <input type="text" name="explain[${num}][video]" id="video${num}" placeholder="请上传附件"  class="layui-input">
		   </div>
	   <button type="button" class="layui-btn upload${num} " data-des="video${num}">
		<i class="layui-icon">&#xe67c;</i>上传附件
	   </button>
	   
	  </div>
	  <button type="button" class="layui-btn layui-btn-danger btn2">删除</button>
  </div>
	  `).appendTo('#xxx');
	  var uploadInst = upload.render({
		 elem: `.upload${num}` //绑定元素
		,url: uploadurl //上传接口
		,field: 'upload' //字段名称
		,multiple: false //多文件上传
	...
	var uploadsInst = upload.render({
		...
		,done: function(res){
			if(res.code==1){
				...
				html += "<dl><dt><img src='"+sitedir+res.data[0]+"' data-url='"+res.data[0]+"'></dt><dd>删除</dd> <input type='text' name='duotutitle[]' value=''  class='layui-input' placeholder='请输入图片名称' style='width:120px;margin-right: 5px;'></dl>";
			}else{
