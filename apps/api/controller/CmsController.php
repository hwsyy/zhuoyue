...
class CmsController extends Controller
{
    ...
    public function canshu(){
        $scode=request('scode');
        $id=request('id');
        $data = $this->model->getSort('cn', $scode);
        $canshu_array=explode('|',$data->canshu);
        $content=$this->model->getContent('cn',$id);


        $canshu_string='';
        foreach($canshu_array as $v){
            $key='ext_'.$v;
            $canshu_string.='<div class="layui-form-item">
                                        <label class="layui-form-label">'.$v.'</label>
                                        <div class="layui-input-block">
                                            <input type="text" name="ext_'.$v.'" value="'.$content->$key.'"  placeholder="" class="layui-input">
                                        </div>
                                    </div>';
        }
        if($data->canshu){
            json(1,$canshu_string);
        }else{
            json(0,'');
        }
    ...
    public function search()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $data[$key]->date=date('Y-m-d',$datetime);
