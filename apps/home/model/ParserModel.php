...
class ParserModel extends Model
{
    ...
    //获取栏目scode
    //
    public function getScode($urlname){
        return parent::table('ay_content_sort')->where("filename='$urlname'")->value('scode');
    }
    ...
    // 自定义标签图片描述，不区分语言，兼容跨语言
    public function getLabelAlt()
    {
        return parent::table('ay_label')->decode()->column('alt_value,type', 'alt');
    }

    //获取国家信息
    public function getCountry(){
        return parent::table('ay_country')->order('name_english')->select();
    }

    //获取总记录数
    public function getTotalRow($scode)
    {
        return parent::table('ay_content')->where("scode='$scode'")->where('status_recycle=1')->count();
    }
    ...
    //获取栏目多图总张数
    public function getTotalRowPicsScode($scode)
    {
        $pics=parent::table('ay_content_sort')->where("scode='$scode'")->value('pics');
        return count(explode(',',$pics));
    }

    //获取栏目所有子栏目
    public function getTotalRowParent($parent)
    {
        return parent::table('ay_content_sort')->where("pcode='$parent'")->count();
        
    }
    //获取列表多图总张数
    public function getTotalRowPicsId($id)
    {
        $pics=parent::table('ay_content')->where("id='$id'")->value('pics');
        return count(explode(',',$pics));
    }


    //获取第一个子类的跳转路径
    public function getFirstLink($scode)
    {
        //var_dump($scode);
        if (! $scode) {
            return;
       ...
       /* $this->scodes[] = $scode;
        ...
        return $this->scodes;*/
        $firstRow=parent::table('ay_content_sort')->where("pcode='$scode'")->order('sorting ASC')->find();
        $this->scodes[] = $firstRow->scode;
        if($firstRow->is_first){
           // var_dump('aaa');
            $this->getFirstLink($firstRow->scode);
           // var_dump('aa');
          //  return $firstRow->scode;

        }
        return $this->scodes;
      //  var_dump($firstRow->scode);
        
    }


    //获取第一个子类数组
    public function getLastArray($scode)
    {
        //var_dump($scode);
        if (! $scode) {
            return;
        ...
        $firstRow=parent::table('ay_content_sort')->where("scode='$scode'")->find();
    ...
    public function getSort($scode)
    {
        ...
        return parent::table('ay_content_sort a')->field($field)
            //->where("a.scode='$scode' OR a.filename='$scode'")
             ->where("a.scode='$scode' OR a.filename like '%$scode%'")
            ->join($join)
            ->find();
    }
    ...
    // 指定分类数量
    public function getSortRows($scode,$keywords='')
    {
        ...
        $where2 = array(
            "acode='" . get_lg() . "'",
            "title like '%" . escape_string($keywords) . "%'",
            'status=1',
    ...
    //获取层级栏目
    // 分类栏目列表关系树
    public function getSortsLevel($level,$mcode)
    {
        $fields = array(
            'a.*',
            'b.type',
    ...
    // 获取二次栏目数量
    public function getSubnum($tcode)
    {
        $result = parent::table('ay_content_sort')->where("pcode=$tcode")->count();
        return $result;
    }

    // 分类二级编码
    public function getSortErScode($scode)
    {
        $scode_link = parent::table('ay_content_sort')->where("scode=$scode")->value('scode_link');
        $scode_link_arr=explode(',',$scode_link);
        if(count($scode_link_arr)==1){
            $ercode=$scode;
        }else{
            $ercode=$scode_link_arr[2];
        }
        return $ercode;
    }
    ...
    // 列表内容,带分页，不区分语言，兼容跨语言
    public function getListsLcode($scode,$lcode,$bcode,$num, $order, $filter = array(), $tags = array(), $select = array(), $fuzzy = true, $start = 1, $lfield = null)
    {
        $ext_table = false;
        if ($lfield) {
            $lfield .= ',id,outlink,type,scode,sortfilename,filename,urlname'; // 附加必须字段
    ...
    // 列表内容，不带分页，不区分语言，兼容跨语言
    public function getListLcode($scode,$lcode,$bcode, $num, $order, $filter = array(), $tags = array(), $select = array(), $fuzzy = true, $start = 1, $lfield = null)
    {
        $ext_table = false;
        if ($lfield) {
            $lfield .= ',id,outlink,type,scode,sortfilename,filename,urlname'; // 附加必须字段
    ...
    public function getContentPics($id)
    {
     ...
     // 指定列表多图
    public function getSortPics($scode)
    {
        $result = parent::table('ay_content_sort')->where("scode='$scode'")
            ->where('status=1')
            ->value('pics');
        return $result;
    }
    ...
    public function getContentPre($scode, $id)
    {
        if (! $this->pre) {
            ...
            $cur=parent::table('ay_content a')->field($field)
    ...
    // 分类栏目列表关系树底部显示
    public function getSortsTreeFoot()
    {
        $fields = array(
            'a.*',
            'b.type',
