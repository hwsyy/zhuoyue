...
class ParserController extends Controller
{
    ...
    public function parserBefore($content)
    {
        ...
        $content = $this->parserUserLabelAlt($content); // 自定义标签图片描述
    ...
    public function parserAfter($content)
    {
        ...
        $content = $this->parserTotalLabel($content);//双循环
        ...
        $content = $this->parserNavFootLabel($content); // 分类列表
        ...
        $content = $this->parserListSubLabel($content); // 指定子列表
        $content = $this->parserSpecifyContentLabel($content); // 指定内容
        $content = $this->parserContentPicsLabel($content); // 内容多图
        $content = $this->parserSortPicsLabel($content);    //列表多图
        $content = $this->parserCountryLabel($content); // 国家调取
        ...
        // $content = $this->parserLeoLabel($content); // 自定义模板语法
        return $content;
    }
    ...
    public function parserSingleLabel($content)
    {
        ...
        $content = str_replace('{pboot:forgetpwd}', Url::home('member/forgetpwd'), $content); // 注册地址
        $content = str_replace('{pboot:resetpwd}', Url::home('member/resetpwd'), $content); // 注册地址
        ...
        $keyword=get('keyword')? :'';
        $content = str_replace('{pboot:keyword}', $keyword, $content); // 关键词搜索


        $tags=get('tag')? :'';
        $content = str_replace('{pboot:tags}', $tags, $content); // tags刷选
        ...
        $content = str_replace('{pboot:qrcode}', CORE_DIR . '/qrcode.php', $content); // 二维码路径
        ...
        $content = str_replace('{pboot:link_title}', $this->config('link_title'), $content); // API认证用户
    ...
    // 解析国家列表标签
    public function parserCountryLabel($content)
    {
        $pattern = '/\{pboot:country(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:country\}/';
        $pattern2 = '/\[country:([\w]+)(\s+[^]]+)?\]/';
                ...
                foreach ($data as $value) { // 按查询数据条数循环
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'name_chinese':
                                if ($value->name_chinese) {
                                   
                                    $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, SITE_DIR . $value->name_chinese), $one_html);
                                ...
                                break;
                            case 'name_english':
                                if ($value->name_english) {
                                   
                                    $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, SITE_DIR . $value->name_english), $one_html);
    ...
    // 解析自定义标签图片描述
    public function parserUserLabelAlt($content)
    {
        $pattern = '/\{labelalt:([\w]+)(\s+[^}]+)?\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $data = $this->model->getLabelAlt();
                ...
                switch ($matches[1][$i]) {
                    default:
                        if (isset($data[$matches[1][$i]])) {
                            ...
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $data[$matches[1][$i]]['alt_value']), $content);
                        }
                }
            }
        }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
          ...
          //  console($data);
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if($level && $mcode){
                    $out_data=$this->model->getSortsLevel($level,$mcode);
                }
              
                 if($start && $num){
                    $out_data = array_slice($out_data, $start,$num);
                }else{
                    if ($num) {
                ...
                // 读取开始截取位置
                    if ($start) {
                        $out_data = array_slice($out_data, $start);
                    }else{
                        $out_data = array_slice($out_data, 0);
                    }
                }
                            ...
                            switch ($matches2[1][$j]) {
                                ...
                                case 'link':
                                    ...
                                    if ($value['outlink']) {
                                        ...
                                        if($value['is_first']){
                                            $lastSubScode=end($this->model->getFirstLink($value['scode']));
                                            $lastSubArray=$this->model->getLastArray($lastSubScode);
                                          //  var_dump($lastSubArray);

                                            $one_html = str_replace($matches2[0][$j], $this->parserLink($lastSubArray->type,  $lastSubArray->urlname, 'list', $lastSubScode, $lastSubArray->filename, '', ''), $one_html);
                                        }else{
    ...
    // 解析底部栏目列表标签
    public function parserNavFootLabel($content)
    {
        $pattern = '/\{pboot:navfoot(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:navfoot\}/';
        $pattern2 = '/\[navfoot:([\w]+)(\s+[^]]+)?\]/';
        $pattern3 = '/pboot:([0-9])+navfoot/';
        if (preg_match_all($pattern, $content, $matches)) {
            $data = $this->model->getSortsTreeFoot();
          //  console($data);
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'tcode': // 顶级栏目ID
                     ...
                     case 'ercode': // 两级栏目ID
                        if (! isset($ercode))
                        $ercode = $this->model->getSortErScode($sort->scode);
                        $content = str_replace($matches[0][$i], $ercode, $content);
                        break;
                    case 'pcode': // 父栏目ID
                        
                            
                        $content = str_replace($matches[0][$i], $sort->pcode, $content);
                        break;
                    case 'subnum': // 次级栏目数量
                       
                        $tcode = $this->model->getSortTopScode($sort->scode);
                        $subnum=$this->model->getSubnum($tcode);
                        $content = str_replace($matches[0][$i], $subnum, $content);
                        break;
                    case 'topname':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                    ...
                    case 'rows':
                        ...
                        $content = str_replace($matches[0][$i], $this->model->getSortRows($sort->scode,$keywords), $content);
                        break;
                    case 'ico':
                        if ($sort->ico) {
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'rows':
                                ...
                                break;
                            case 'navRows':
                                $one_html = str_replace($matches2[0][$j], $this->model->getSortNavRows($value->scode), $one_html); // 获取分类包含子类的内容数量
                                break;
                            default:
                                if (isset($value->{$matches2[1][$j]})) {
                                    $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, $value->{$matches2[1][$j]}), $one_html);
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'link':
                                ...
                                $qs[$field] = $value;
                                $qs = urldecode(http_build_query($qs));
    ...
    public function parserListLabel($content, $cscode = '')
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                $num = 1000; // 未设置条数时使用默认15
                $order = 'a.sorting ASC,a.date DESC,a.id DESC'; // 默认排序
                $filter = ''; // 过滤
                $tags = ''; // tag标签
               ...
               // 采取keyword方式
                ...
                if ($page) {
                    // title数据传值筛选
                    if (! ! $get_title = get('title', 'vars')) {
                        if ($fuzzy) {
                            $where2[] = "a.title like '%" . $get_title . "%'";
                        } else {
                            $where2[] = "a.title='" . $get_title . "'";
                        }
                    }
                ...
                if($ext){
                    $ext_key=explode('=',$ext)[0];
                    $ext_value=explode('=',$ext)[1];
                    $where3[$ext_key]=$ext_value;
                }
    ...
    // 解析内容子列表标签
    public function parserListSubLabel($content, $cscode = '')
    {
        $pattern = '/\{pboot:sublist(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:sublist\}/';
        $pattern2 = '/\[sublist:([\w\+\-\*\/\%]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
               ...
               // 采取keyword方式
                ...
                if ($page) {
                    if (isset($paging)) {
                        ...
                        $data = $this->model->getListsLcode($scode,$lcode,$bcode,$num, $order, $where1, $where2, $where3, $fuzzy, $start, $lfield);
                    }
                } else {
                    $data = $this->model->getListLcode($scode,$lcode,$bcode,$num, $order, $where1, $where2, $where3, $fuzzy, $start, $lfield);
                }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (! ! $pics = $this->model->getContentPics(escape_string($id))) {
                    $pics = explode(',', $pics);
                    $pics =array_slice($pics,$start,$num);
                    $duotutitle = \core\basic\Db::table('ay_content')->where("id='$id'")->where('status=1')->value('duotutitle');
                    $duotutitle = explode(',', $duotutitle);
                    $duotutitle =array_slice($duotutitle,$start,$num);
                } else {
                    $pics = array();
                    $duotutitle = [];
                }
                ...
                $out_html = '';
                $key = 1;
                foreach ($pics as $pic_key => $value) { // 按查询图片条数循环
                    $one_html = $matches[2][$i];
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        $params = $this->parserParam($matches2[2][$j]);
                        switch ($matches2[1][$j]) {
                            ...
                            case 'i':
                                ...
                                break;
                            case 'duotutitle':
                                $duotutitle_str = empty($duotutitle[$pic_key])?'':$duotutitle[$pic_key];
                                $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$duotutitle_str), $one_html);
//                                $one_html = $matches2[0][$j];
                                break;
                            case 'src':
                                if ($value) {
    ...
    // 解析列表多图
    public function parserSortPicsLabel($content)
    {
        $pattern = '/\{pboot:sortpics(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:sortpics\}/';
        $pattern2 = '/\[sortpics:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                // 获取调节参数
                ...
                // 读取内容多图
                if (! ! $pics = $this->model->getSortPics(escape_string($scode))) {
                    $pics = explode(',', $pics);
                    $pics =array_slice($pics,$start,$num);
                    $duotutitle = \core\basic\Db::table('ay_content_sort')->where("scode='$scode'")->where('status=1')->value('duotutitle');
                    $duotutitle = explode(',', $duotutitle);
                    $duotutitle =array_slice($duotutitle,$start,$num);
                } else {
                    $pics = array();
...
//                                $one_html = $matches2[0][$j];
                ...
                foreach ($data as $value) { // 按查询数据条数循环
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'src2':
                                if ($value->pic2) {
                                    if (! preg_match('/^http/', $value->pic2)) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, SITE_DIR . $value->pic2), $one_html);
                                    } else {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, $value->pic2), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                for ($n = $start; $n <= $end; $n ++) {
                    ...
                    $one_html = str_replace('[loop:number]', $n*$number, $one_html);
                    $out_html .= $one_html;
                    $key ++;
                }
                $content = str_replace($matches[0][$i], $out_html, $content);
    ...
    // 解析循环标签
    public function parserTotalLabel($content)
    {
        $pattern = '/\{pboot:total(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:total\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                // 获取调节参数
                ...
                if($ispics){
                    if($scode){
                        $totalrow=$this->model->getTotalRowPicsScode($scode);
                    }else{
                        $totalrow=$this->model->getTotalRowPicsId($id);
                    }
                }else{
                    $totalrow=$this->model->getTotalRow($scode);
                }

                if($parent){
                    $totalrow=$this->model->getTotalRowParent($parent);
                }
                
              
               
                $page=(int)ceil($totalrow/$num);
               
                $out_html = '';
                $key = 1;
                for ($n = 0; $n < $page; $n ++) {
                    $one_html = str_replace('[total:n]', $key - 1, $matches[2][$i]);
                    $one_html = str_replace('[total:i]', $key, $one_html);
                    $one_html = str_replace('[total:index]', $n, $one_html);
                    $one_html = str_replace('[total:number]', $n*$num, $one_html);
                    $out_html .= $one_html;
                    $key ++;
                }
                $content = str_replace($matches[0][$i], $out_html, $content);
        ...
        foreach ($params as $key => $value) {
            switch ($key) {
                ...
                case 'lencn': // 以中文占位长度方式截取，英文算半个
                    ...
                    break;
                case 'substr': // 以中文占位长度方式截取，英文算半个
                    if ($params['substr'] && is_string($data)) {
                        $substr=$params['substr'];
                        $index=explode(',',$substr)[0];
                        $num=explode(',',$substr)[1];
                        $data = substr($data, $index, $num);
                    }
                    break;
    ...
    protected function parserList($label, $search, $content, $data, $params, $key)
    {
        switch ($label) {
            ...
            case 'k':
                ...
                break; 
            case 'ling':
                if($this->adjustLabelData($params, $key)<10){
                     $content = str_replace($search, '0'.$this->adjustLabelData($params, $key), $content);
                }else{
                     $content = str_replace($search, $this->adjustLabelData($params, $key), $content);
                } 
                break;   
            ...
            case 'enclosuresize2':
                if ($data->enclosure && file_exists(ROOT_PATH . $data->enclosure)) {
                    $content = str_replace($search,round(filesize(ROOT_PATH . $data->enclosure) / 1048576 * 100) / 100 . ' MB', $content);
                } else {
                    $content = str_replace($search, 0, $content);
                }
            case 'likeslink':
    ...
    protected function parserContent($label, $search, $content, $data, $params, $sort)
    {
        switch ($label) {
            ...
            case 'precontent':
                ...
                if (! ! $pre = $this->model->getContentPre($sort->scode, $data->id)) {
                    ...
                    if (isset($params['notext'])) {
                        ...
                        $content = str_replace($search, $this->config('nomore'), $content);
                    }
                }
                break;
            case 'prelink':
            ...
            case 'keywords': // 如果内容关键字为空，则自动使用全局关键字
                ...
                break;
            case 'pro_link': // 如果内容关键字为空，则自动使用全局关键字
                if ($data->pro_link) {
                    $content = str_replace($search, $this->adjustLabelData($params, $data->pro_link), $content);
                } else {
                    $content = str_replace($search, '', $content);
                }
                break;
            case 'doc_link': // 如果内容关键字为空，则自动使用全局关键字
                if ($data->doc_link) {
                    $content = str_replace($search, $this->adjustLabelData($params,$data->doc_link), $content);
                } else {
                    $content = str_replace($search,'', $content);
                }
                break;         
    ...
    protected function parserLink($type, $urlname, $page, $scode, $sortfilename, $id, $contentfilename)
    {
        ...
        if ($type == 1) {
            ...
            if ($sortfilename) {
                $link = Url::home($sortfilename, false);
            } else {
                $link = Url::home($urlname . $url_break_char . $scode, false);
            }
        } else {
    ...
    protected function checkLabelLevel($params)
    {
        ...
        return true;
            ...
            // 自定义标签 //关联内容
    public function parserLeoLabel($content)
    {
        $pattern = '/\{leo:leo(\s+[^}]+)?\}([\s\S]*?)\{\/leo:leo\}/';
        $pattern2 = '/\[leo:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                ...
                if($params['link']){
                $case_link = explode(",",$params['link']);
                foreach ($case_link as $key => $value) {
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'title':
                                if ($value->title) {
                                    if ($value->title) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$value->title), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
                            ...
                            case 'enclosure':
                                if ($value->enclosure) {
                                    if ($value->enclosure) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$value->enclosure), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
                            ...
                            case 'date':
                                if ($value->date) {
                                    if ($value->date) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$value->date), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
                                break;  
                            case 'ext_solution_desc':
                                if ($value->ext_solution_desc) {
                                    if ($value->ext_solution_desc) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$value->ext_solution_desc), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
                            ...
                            case 'link':
                            if ($value->type == 1) {
                                $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$this->parserLink($value->type, $value->urlname, 'about', $value->scode, $value->sortfilename, '', '')), $one_html);  
                                } else {
        ...
        return $content;
                ...
                // 自定义标签 //关联内容
    public function parserLeo2Label($content)
    {
        $pattern = '/\{leo2:leo2(\s+[^}]+)?\}([\s\S]*?)\{\/leo2:leo2\}/';
        $pattern2 = '/\[leo2:([\w]+)(\s+[^]]+)?\]/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'enclosuresize':
                                if ($value->enclosure) {
                                    if ($value->enclosure) {
                                        $enclosuresize = round(filesize(ROOT_PATH . $value->enclosure) / 1048576 * 100) / 100 . ' MB';
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params,$enclosuresize), $one_html);
                                    }
                                } else {
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);
                                }
