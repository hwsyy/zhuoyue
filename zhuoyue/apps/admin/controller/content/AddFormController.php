...
class AddFormController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->display('content/addform.html');
    }
        ...
        if ($_POST) {
         ...
         //   exit;
           ...
           /* // 字段不存在时创建
            ...
            $result = $this->model->amd("ALTER TABLE $table ADD COLUMN $name TEXT($max) default $value");
            if($result){
                success('新增成功！', url('/admin/AddForm/index'));
            }else{
                $this->log('新增字段失败！');
                error('新增失败！', - 1);
            }
