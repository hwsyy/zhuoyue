...
class Paging
{
    ...
    private function pageNumBar()
    {
        ...
        $num_html = '';
        if ($this->page > $halfu) {
           /* $num_html.='<a href="' . $this->buildPath(1) . '">1</a>';*/
            $num_html .= '<a class="page-num">···</a>';
        }
        ...
        if ($this->page <= $halfl || $this->pageCount < $total) { // 当前页小于一半或页数小于总数
            for ($i = 1; $i <= $total; $i ++) {
                if ($i > $this->pageCount)
                    break;
                if ($this->page == $i) {
                    $num_html .= '<a href="' . $this->buildPath($i) . '" class="active">' . $i . '</a>';
                } else {
                    $num_html .= '<a href="' . $this->buildPath($i) . '" class="">' . $i . '</a>';
                }
            }
        ...
        if ($this->pageCount > $total && $this->page < $this->pageCount - $halfl) {
            $num_html .= '<a class="page-num">···</a>';
           /* $num_html.='<a href="' .$this->buildPath($this->pageCount) . '">'.$this->pageCount.'</a>';*/
        }
