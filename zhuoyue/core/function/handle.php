...
//腾讯云发送短信通知
function sendWithParam($nationCode, $phoneNumber,$random, $params, $sign = "", $extend = "", $ext = ""){
         
         $appid = Config::get('sms_appid');  //自己的短信appid
         $appkey = Config::get('sms_secret');//自己的短信appkey
         $templId=Config::get('sms_checkcodetpl');
         $sign=Config::get('sms_signame');
...
function get_last_url(){
    $url=get_current_url();
    $url_arr=explode('/',$url);
    $count=count($url_arr)-1;
    $url_last=$url_arr[$count];
    $url_last=rtrim($url_last,'.html');
    return $url_last;
}
...
function create_code_number($len = 6)
{
    $charset = '1234567890';
    $charset = str_shuffle($charset);
    $charlen = strlen($charset) - 1;
