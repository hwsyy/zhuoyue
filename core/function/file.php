...
//繁体转简体
function tranJian($content){
    $fanti=Config::get('fanti');
    $jianti=Config::get('jianti');
    $length=strlen($content);
        
    for ($x=0; $x<=$length; $x++) {
        
      if($index=strpos($fanti,mb_substr($content,$x,1))){
       $index=$index/3;
      
        $content=str_replace(mb_substr($fanti,$index,1),mb_substr($jianti,$index,1),$content);
      }
    } 
}
...
function downPic($content){
    ...
    foreach($allImg as $k=>$v){
        ...
        $newv=$domain.'/weixin.php?url='.$v;

        $content=str_replace($v,$newv,$content);
    }

}


function getWeixinInfo($url){
$html =file_get_contents($url);


//内容
$str_16 = substr($html,strripos($html,'div class="rich_media_content')+100);

$content = substr($str_16,0,strrpos($str_16,"first_sceen__time")-75);
...
$title=clear_html_blank($title[1][0]);

//作者
$str_14 = substr($html,0,strrpos($html,"og:title")-27);

$author = clear_html_blank(substr($str_14,strripos($str_14,"author")+17)); //公众号文章作者


//公众号名称
preg_match_all("/id=\"js_name\">(.*)<\/a>/iUs",$html,$gongzhong,PREG_PATTERN_ORDER);

$gongzhong=clear_html_blank($gongzhong[1][0]);


//发布时间
preg_match ( '/var\s+ct\s*=\s*"([^\s]*)"/', $html, $result );
$date=date('Y-m-d H:i:s',$result[1]);
if(!$date){
    $date=date('Y-m-d H:i:s',time());
}

//公众号封面图
preg_match ( '/var\s+msg_cdn_url\s*=\s*"([^\s]*)"/', $html, $ico );

$ico=$domain.'/weixin.php?url='.$ico[1];
...
if($title){
   ...
   $data=array_merge($data,$titlearr);
...
if($author){
   ...
   $data=array_merge($data,$authorarr);
}


if($gongzhong){
     $sourcearr=array('source'=>$gongzhong);
  $data= array_merge($data,$sourcearr);
}

if($date){
     $datearr=array('date'=>$date);
  $data= array_merge($data,$datearr);
}

if($ico){
     $icoarr=array('ico'=>$ico);
   $data=array_merge($data,$icoarr);
}
...
function saveImage($url)
{
    ...
    $imgName=time().mt_rand(1000,9999);

   
    $return_content = http_get_data($url);  
    $filename = 'static/weixin/'.$imgName.'.'.$ext;  
...
function upload($input_name, $file_ext = null, $max_width = null, $max_height = null, $watermark = false)
{
    ...
    $upload_ext = Config::get('home_upload_ext');
    array_push($array_ext_allow,$upload_ext);
...
function watermark_img($src_image, $out_image = null, $position = null, $watermark_image = null, $watermark_text = '', $watermark_text_size = null, $watermark_text_color = null)
{
    ...
    // 如果不存在文字及图片则直接返回
    if (! $watermark_text) {
        $watermark_text = Config::get('watermark_text') ?: 'leo';
    }
