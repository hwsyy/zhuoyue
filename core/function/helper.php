...
function guolv($str){
     $str=strip_tags($str);
     $reg='echo|return|alert|eval|console';
     $pattern = '/'.$reg.'/Ui';
 ...
 */
function error($string, $jump_url = null, $time = 1)
{
    @ob_clean();
    if (! $string)
        $string = '未知错误！';
    ...
    if (Config::get('return_data_type') == 'json' || is_ajax()) { // 接口模型返回格式数据
        ...
        $err_tpl = ROOT_PATH . '/404/404.html';
 ...
 * @param number $_time时间
 */
function success($string, $jump_url = null, $time = 1)
{
    if ($jump_url == '-1' && isset($_SERVER['HTTP_REFERER'])) {
        $jump_url = $_SERVER['HTTP_REFERER'];
        if (strpos($jump_url, get_http_url()) !== 0) {
...
function _404($string, $jump_url = null, $time = 2)
{
    ...
    $file_404 = ROOT_PATH . '/404/404.html';
